...
def build_tools(graph: CircuitGraph) -> list:
    ...
    def get_first_component() -> str:
        """Retourne le composant le plus proche de la source de tension/pile (le 'premier' composant du circuit, mesuré en nombre de connexions depuis la source)."""
        src = _find_source_id(graph)
        if src is None:
            return "aucune source de tension trouvée dans ce circuit"
        G = _to_networkx(graph)
        distances = {nid: d for nid, d in nx.single_source_shortest_path_length(G, src).items()
                    if nid != src}
        if not distances:
            return "aucun composant connecté à la source"
        min_d = min(distances.values())
        closest = sorted(nid for nid, d in distances.items() if d == min_d)
        return closest[0] if len(closest) == 1 else ", ".join(closest)

    def get_last_component() -> str:
        """Retourne le composant le plus éloigné de la source de tension/pile (le 'dernier' composant du circuit, mesuré en nombre de connexions depuis la source)."""
        src = _find_source_id(graph)
        if src is None:
            return "aucune source de tension trouvée dans ce circuit"
        G = _to_networkx(graph)
        ...
        farthest = sorted(nid for nid, d in distances.items() if d == max_d)
        return farthest[0] if len(farthest) == 1 else ", ".join(farthest)
